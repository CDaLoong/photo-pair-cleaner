...
- 评分整理先生成可筛选、可展开的只读计划，展示成员快照、目标路径、预计空间和冲突原因；用户只可勾选计划就绪的复制/移动照片组，并在二次确认后执行。
- 复制先写入同目录临时文件并校验 SHA-256，再以不覆盖方式提交；同盘移动优先原子重命名，跨盘移动会先完成全部目标复制与校验，再删除源文件。
- 评分整理历史保存在应用数据目录，记录规则、同步偏好、逐组结果与文件摘要；未变化的移动目标可以恢复，未变化的复制目标可以撤销，原位置被占用时整组移动恢复暂停。
- 待清理照片组当前仍只进入计划并标注“第五阶段开放”，不会由评分整理执行接口移入隔离区或系统回收站。
...
15. 评分整理执行只能消费一次后端计划，只允许计划就绪的复制或移动照片组；前端不能提交动作或文件路径。
16. 复制和跨盘移动使用临时文件、内容摘要与无覆盖提交；跨盘移动在全部目标校验完成前不会删除任何源文件。
17. 操作历史清单不可覆盖，恢复结果单独追加；移动恢复和复制撤销都会重新计算文件摘要，已变化文件保持原状。
18. 评分整理的待清理动作仍不可执行；现有配对清理的隔离区/回收站入口不会接受评分整理计划。
...
当前版本不包含 RAW 解码、AI 筛片、评分待清理计划的执行或 Lightroom 目录数据库解析。Lightroom Classic 外部交接会启动应用并传入照片，但不会定位或修改既有 `.lrcat` 目录记录。
