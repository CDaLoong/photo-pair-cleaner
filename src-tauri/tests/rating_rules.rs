...
fn names_and_member_scopes_must_be_actionable() {
    ...
    assert!(
        validate_rule_set(&[duplicate_scope])
            .unwrap_err()
            .contains("处理格式不能重复")
    );
}
...
fn rating_conditions_stay_between_zero_and_five() {
    ...
    assert!(
        validate_rule_set(&[invalid_equal])
            .unwrap_err()
...
fn destinations_are_required_only_for_copy_and_move() {
    ...
    assert!(
        validate_rule_set(&[missing_move])
            .unwrap_err()
    ...
    assert!(
        validate_rule_set(&[missing_copy])
            .unwrap_err()
...
fn damaged_existing_database_is_never_silently_overwritten() {
    ...
    assert!(
        save_rules(&database, &[rule("safe", RuleAction::Keep)])
            .unwrap_err()
            .contains("评分规则已损坏")
    );
...
fn export_requires_json_and_round_trips_the_same_envelope() {
    ...
    assert!(
        export_rules(&invalid, &[rule("one", RuleAction::Keep)])
            .unwrap_err()
...
fn symlinked_databases_and_exports_are_rejected() {
    ...
    assert!(
        export_rules(&link, &[rule("one", RuleAction::Keep)])
            .unwrap_err()
